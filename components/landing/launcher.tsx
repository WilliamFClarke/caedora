...
/**
 * Minimal launcher used inside the Electron desktop shell — bypasses the
 * marketing/product page entirely and goes straight to "create a vault" or
 * "open an existing one", same as the original web home before the redesign.
 */
    ...
    <Shell>
      <div className="flex w-full max-w-md flex-col items-center gap-8 text-center">
        <div className="flex items-center gap-3">
          <Image src="/caedora-logo.png" alt="Caedora" width={48} height={48} priority />
        ...
        {status.state === 'permission-required' ? (
          <div className="flex max-w-sm flex-col items-center gap-4">
            <p className="text-muted-foreground text-sm">
              Re-grant access to <span className="font-medium">{status.folderName}</span> to
              continue.
            </p>
...
function VaultRow({
  ...
  const subtitle = isGithub
    ...
    : vault.state.directoryPath
      ? 'Desktop folder'
      : 'Local folder'
  const lastOpened = vault.state.lastOpenedAt ? timeAgo(vault.state.lastOpenedAt) : 'never'
